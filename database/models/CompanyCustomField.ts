...
export type CompanyCustomFieldType = 'text' | 'number' | 'date' | 'boolean' | 'select' | 'multiselect';

export interface ICompanyCustomField extends Document {
  name: string;
  label: string;
...
const CompanyCustomFieldSchema = new Schema<ICompanyCustomField>(
  {
    name: {
      type: String,
      required: true,
...
// Methods
CompanyCustomFieldSchema.methods.validateValue = function (value: any): boolean {
...
CompanyCustomFieldSchema.methods.formatValue = function (value: any): any {
  if (value === undefined || value === null || value === '') {
...
// Indexes
CompanyCustomFieldSchema.index({ name: 1 }, { unique: true });
CompanyCustomFieldSchema.index({ isActive: 1 });
CompanyCustomFieldSchema.index({ type: 1 });
CompanyCustomFieldSchema.index({ required: 1 });

export default mongoose.models.CompanyCustomField || mongoose.model<ICompanyCustomField>('CompanyCustomField', CompanyCustomFieldSchema);

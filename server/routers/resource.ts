...
export const resourceRouter = createTRPCRouter({
  ...
  getContents: protectedProcedure
    .input(
      z
        .object({
          ...
          visibility: z
            .array(z.enum(['PUBLIC', 'SHARED', 'PRIVATE']))
            .optional(),
        })
        .optional()
    )
    .query(async ({ ctx, input }) => {
      ...
      let conditions = or(
        ...
        and(
          eq(resourceContent.visibility, 'SHARED'),
          eq(resourceContentShare.sharedWithUserId, userId)
        )
      );
      ...
      if (input?.visibility) {
        conditions = and(
          conditions,
          inArray(resourceContent.visibility, input.visibility)
        );
      }
      ...
      if (input?.search) {
        conditions = and(
          conditions,
          or(
            like(resourceContent.title, `%${input.search}%`),
            like(resourceContent.description, `%${input.search}%`)
          )
        );
      }
      ...
      const result = await ctx.db
        .select({
          ...
          lastSentAt:
            sql<Date | null>`max(${resourceContentSendTrack.sentAt})`.mapWith(
              (d) => d && new Date(d)
            ),
          recipients: sql<any[]>`
            json_agg(
        ...
        .leftJoin(
          resourceContentShare,
          eq(resourceContent.id, resourceContentShare.resourceId)
        )
        .leftJoin(
          resourceContentSendTrack,
          eq(resourceContent.id, resourceContentSendTrack.resourceId)
        )
  ...
  getContent: protectedProcedure
    .input(z.object({ id: z.string(), includeShare: z.boolean().optional() }))
    .query(async ({ ctx, input }) => {
      const userId = ctx.session.user.id;
          ...
          and(
            ...
            or(
              ...
              and(
                eq(resourceContent.visibility, 'SHARED'),
                eq(resourceContentShare.sharedWithUserId, userId),
                eq(resourceContentShare.permission, 'edit')
              )
            )
          )
        )
  ...
  deleteContent: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return ctx.db
        ...
        .where(eq(resourceContent.id, input));
    }),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existing[0].visibility === 'PRIVATE') {
        await ctx.db
          ...
          .where(eq(resourceContent.id, input.resourceId));
      }

      await ctx.db
        .delete(resourceContentShare)
        .where(
          and(
            eq(resourceContentShare.resourceId, input.resourceId),
            inArray(resourceContentShare.sharedWithUserId, input.userIds)
          )
        );
  ...
  removeShare: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!existing[0]) {
        throw new TRPCError({
          code: 'UNAUTHORIZED',
          message: 'Not authorized to modify shares for this content',
        });
      }
      ...
      return ctx.db
        ...
        .where(
          and(
            eq(resourceContentShare.resourceId, input.resourceId),
            eq(resourceContentShare.sharedWithUserId, input.userId)
          )
        );
    }),

  createEmail: protectedProcedure
    .input(resourceEmailSchema)
    .mutation(async ({ ctx, input }) => {
  ...
  getEmails: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const userId = ctx.session.user.id;
      let conditions = or(
        eq(resourceEmails.createdBy, userId),
        eq(resourceEmails.visibility, 'PUBLIC')
      );

      if (input?.visibility) {
        conditions = and(
          conditions,
          eq(resourceEmails.visibility, input.visibility)
        );
      }
      ...
      if (input?.search) {
        conditions = and(
          conditions,
          or(
            like(resourceEmails.title, `%${input.search}%`),
            like(resourceEmails.description, `%${input.search}%`),
            like(resourceEmails.subject, `%${input.search}%`)
          )
        );
      }
  ...
  updateEmail: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!existing[0]) {
        throw new TRPCError({
          code: 'UNAUTHORIZED',
          message: 'Not authorized to edit this email template',
        });
      }

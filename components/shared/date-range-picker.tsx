...
export function DateRangePicker({ className, initialDateFrom, initialDateTo, onChange, align = 'start' }: DateRangePickerProps) {
  ...
  const moveMonth = (direction: 'forward' | 'backward') => {
    setCurrentMonth((current) => {
      return direction === 'forward' ? addMonths(current, 1) : addMonths(current, -1);
    });
  };
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <Button
            id='date'
            variant={'outline'}
            className={cn('w-[300px] justify-start text-left font-normal', !selectedRange && 'text-muted-foreground', 'hover:bg-accent hover:text-accent-foreground')}
          >
